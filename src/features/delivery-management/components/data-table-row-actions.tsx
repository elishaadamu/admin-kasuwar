...
import { UserCheck, UserX, Trash2, Eye, Check } from 'lucide-react'
...
export function DataTableRowActions({ row }: DataTableRowActionsProps) {
  ...
  const handleStatusChange = async (status: 'active' | 'suspended') => {
    ...
    const endpoint = isSuspending
      ? API_CONFIG.ENDPOINTS.DELIVERY_MANAGEMENT.SUSPEND
      : API_CONFIG.ENDPOINTS.DELIVERY_MANAGEMENT.UNSUSPEND
  ...
  const handleApprove = async () => {
    const prevApproved = user.isApproved
    // Optimistic update
    updateUser(user._id, { isApproved: true })

    try {
      await toast.promise(
        axios.put(
          `${apiUrl(API_CONFIG.ENDPOINTS.DELIVERY_MANAGEMENT.APPROVE)}${authUser.id}/${user._id}`
        ),
        {
          loading: `Approving delivery man...`,
          success: `Delivery man has been approved.`,
          error: `Failed to approve delivery man.`,
        }
      )
    } catch (err) {
      // Revert on error
      updateUser(user._id, { isApproved: prevApproved })
    }
  }
