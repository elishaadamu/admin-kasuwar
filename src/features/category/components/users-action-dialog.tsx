    ...
    try {
      ...
      if (isEdit) {
        const response = await axios.put(
          `${apiUrl(API_CONFIG.ENDPOINTS.CATEGORY.UPDATE)}${user?.id}/${currentRow?._id}`,
          payload
        )
        ...
        const response = await axios.post(
          `${apiUrl(API_CONFIG.ENDPOINTS.CATEGORY.CREATE)}${user?.id}`,
          payload
        )
      ...
      const errorMessage =
        ...
        (isEdit ? 'Failed to update category.' : 'Failed to add new category.')
