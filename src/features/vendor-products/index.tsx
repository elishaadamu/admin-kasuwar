...
const route = getRouteApi('/_authenticated/vendor-products/')
  ...
  const fetchUsers = async () => {
    ...
    try {
      const response = await axios.get(
        apiUrl(API_CONFIG.ENDPOINTS.PRODUCT.GET) + user?.id
      )
      console.log(response.data)

      const allProducts = response.data?.products || []
      const vendorProducts = allProducts.filter(
        (product: any) => product.addedBy === 'vendor'
      )

      setUsers(vendorProducts)
    } catch (error) {
        ...
        <div className='mb-2 flex flex-wrap items-center justify-between space-y-2'>
          <div>
            ...
            <p className='text-muted-foreground'>
              Manage vendors Products and their details here.
            </p>
          </div>
        </div>
