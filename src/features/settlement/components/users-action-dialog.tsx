  ...
  const onSubmit = async (values: UserForm) => {
    ...
    try {
      if (isEdit) {
        ...
        showSubmittedData(values, 'Vendors updated:')
      ...
      const errorMessage =
        ...
        (isEdit ? 'Failed to update vendors.' : 'Failed to add new vendors.')
    ...
    >
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader className='text-start'>
          <DialogTitle>
            {isEdit ? 'Edit vendors' : 'Add New vendors'}
          </DialogTitle>
          <DialogDescription>
            {isEdit ? 'Update the vendors here. ' : 'Create new vendors here. '}
        ...
        <p className='mb-4 text-center text-sm text-gray-500'>
          Note: Password is the vendor Phone number
        </p>
      </DialogContent>
    </Dialog>
  )
